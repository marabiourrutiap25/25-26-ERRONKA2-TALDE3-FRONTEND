...
const props = defineProps({
  title: { type: String, default: "Editatu" },
...
const setFormData = (data) => {
  Object.keys(data).forEach(key => {
    if (form[key] !== undefined) form[key] = data[key]
  })
}
...
const onSubmit = () => { emit("submit", { ...form }); close() }
...
.btn-save {
  background-color: #1d7eda; /* El azul de la imagen */
  border-radius: 50px;
  color: white;
  font-weight: 500;
  border: none;
  box-shadow: 0 4px 6px rgba(29, 126, 218, 0.2);
}
